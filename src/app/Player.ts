...
class Player {
  ...
  constructor(controls: PointerLockControls) {
    ...
    this.speed = new THREE.Vector3(45000, 45000, 45000);
  ...
  private async initVoice() {
    /* disabled and not used. maybe never fully worked and relys on old libs.
    try {
      await fetch('https://ecosystems-voice.herokuapp.com/getmodel');
      await fetch('https://ecosystems-voice.herokuapp.com/getmodel');

      const voiceModel =
        await tf.loadLayersModel('https://ecosystems-voice.herokuapp.com/model.json');
      ...
      const voiceModel = await tf.loadLayersModel('localstorage://voice-model');
      this.voice = new Voice(voiceModel);
      await this.voice.init();
    }
    */
