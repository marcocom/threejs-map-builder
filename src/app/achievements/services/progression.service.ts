...
class ProgressionService {
  ...
  async init(): Promise<void> {
    // console.log(`progression.service init() progression:`, storageSvc.get(STORAGES_KEY.progression));
    if (!storageSvc.get<Object>(STORAGES_KEY.progression)) {
      this.storage = getProgressionStorage();
      await storageSvc.set<Object>(STORAGES_KEY.progression, this.storage);
    }

    // console.log(`progression.service init() trophies:`, storageSvc.get(STORAGES_KEY.trophies));
    if (!storageSvc.get<Object>(STORAGES_KEY.trophies)) {
      await storageSvc.set<Object>(STORAGES_KEY.trophies, {});
    }

    // console.log(`progression.service init() completed:`, storageSvc.get(STORAGES_KEY.completed));
    if (!storageSvc.get<Object>(STORAGES_KEY.completed)) {
      await storageSvc.set<Object>(STORAGES_KEY.completed, []);
    }
  }
