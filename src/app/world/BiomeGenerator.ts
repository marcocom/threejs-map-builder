...
class BiomeGenerator {
    ...
    switch (terrain.getWorld().getSeed().toLowerCase()) {
      ...
      default:
        ...
        for (const item of Biomes) {
          temp += item.weight / sum;
          if (rand <= temp) {
            biomeClass = item.class;
            break;
          }
        }
  ...
  pick(x: number, z: number, parameters: IPickObject = {}): IPick | null {
    ...
    for (const item of organisms) { // dual-counters!
      temp += item.weight;
  ...
  computeElevationAt(x: number, z: number): number {
    ...
    const TERRAIN_HEIGHT_DIVIDER = 0.1;
    const GROUND_FALLBACK_CALC = 128;
    // clamp to a minimum elevation
    const minElevation = -(Chunk.HEIGHT / HEIGHT_DIVIDER) / Chunk.MAX_TERRAIN_HEIGHT + TERRAIN_HEIGHT_DIVIDER;

    if (e < minElevation) {
      // default ground fallback
      const nx = x / (1024 * GROUND_FALLBACK_CALC);
      const nz = z / (1024 * GROUND_FALLBACK_CALC);
  ...
  computeWaterHeightAt(x, z) {
    ...
    let e = Number(this.noise2(nx, nz));
