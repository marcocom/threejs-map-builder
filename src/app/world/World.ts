...
class World {
  ...
  static readonly FOG_COLOR: THREE.Color = new THREE.Color(0xcbe2ff);

  static readonly AMBIENT_SOUND_VOLUME: number = 0.35;
   ...
   */
  constructor(scene: THREE.Scene, camera: THREE.PerspectiveCamera, controls: PointerLockControls) {
    this.scene = scene;
    this.camera = camera;
  ...
  async init(seed: string = MathUtils.randomUint32().toString()): Promise<string> {
    this.initSeed(seed);
    ...
    await this.player.init(spawn, target);
    ...
    try {
      ...
      console.log('World.ts :: audio initialization fail');
    }
  ...
  private initFog() {
    if (World.SHOW_FOG) {
      ...
      this.scene.fog = new THREE.Fog(new THREE.Color(0x000000), near, far);
    }
  }
  ...
  private async initAudio() {
    ...
    this.audioLoader.load(ambientSoundPath, (buffer) => {
      ...
      this.zSound.setVolume(configSvc.soundEnabled ? World.AMBIENT_SOUND_VOLUME : 0);
      this.zSound.play();
    }, () => { }, () => { });
    configSvc.soundEnabled$.subscribe(enabled => this.zSound.setVolume(enabled ? World.AMBIENT_SOUND_VOLUME : 0));
    ...
    if (configSvc.debug) {
      ...
      const material = new THREE.MeshPhongMaterial({ color: new THREE.Color(0xff2200) });
    ...
    console.log(`position:: x:${object.position.x} y:${object.position.y} z:${object.position.z}`);
  ...
  */
  ...
  static pointInWorld(point: THREE.Vector3): boolean {
    ...
    return MathUtils.between(
      point.x,
      0 + margin,
      Terrain.SIZE_X - margin
    ) &&
      MathUtils.between(
        point.z,
        0 + margin,
        Terrain.SIZE_Z - margin
      );
  }
}
