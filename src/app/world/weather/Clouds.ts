...
const SNOW = {
  material: new THREE.PointsMaterial({
    size: 2048,
    map: CommonUtils.createSnowFlakeTexture('$color-pearl'),
...
class Clouds {
  ...
  init(scene: THREE.Scene) {
    ...
    if (configSvc.debug) {
      const arrowHelper = new THREE.ArrowHelper(this.wind, new THREE.Vector3(Terrain.SIZE_X / 2, Chunk.CLOUD_LEVEL, Terrain.SIZE_Z / 2), 10000, new THREE.Color(0xff0000));
