...
class Weather {
  ...
  private static PHASES: IPhase[] = [
    { name: 'dawn', value: 0, daylight: 0.5, boundLightIntensity: 0.5, fogColor: '#475d73', skyColor: '#789bc2', groundColor: '#ec70c5' }, // dawn
    { name: 'day', value: 22, daylight: 0.75, boundLightIntensity: 0.0, fogColor: '#becfde', skyColor: '#789bc2', groundColor: '$color-swamp' },
    { name: 'day', value: 157, daylight: 1.0, boundLightIntensity: 0.0, fogColor: '#becfde', skyColor: '#789bc2', groundColor: '$color-swamp' },
    { name: 'dusk', value: 180, daylight: 0.75, boundLightIntensity: 0.5, fogColor: '#475d73', skyColor: '#789bc2', groundColor: '#e86b4a' }, // dusk
    { name: 'night', value: 202, daylight: 0.0, boundLightIntensity: 0.0, fogColor: '#212C37', skyColor: '#789bc2', groundColor: '$color-swamp' },
    { name: 'night', value: 337, daylight: 0.0, boundLightIntensity: 0.0, fogColor: '#212C37', skyColor: '#789bc2', groundColor: '$color-swamp' },
  ];
  ...
  */
  constructor(scene: THREE.Scene, generator: BiomeGenerator) {
    ...
    this.sunAngle = THREE.MathUtils.degToRad(startAngle);
  ...
  */
  ...
  initLights() {
    ...
    this.hemisphereLight = new THREE.HemisphereLight(new THREE.Color(0x3a6aa0), new THREE.Color(0xffffff), 0.5);
    this.hemisphereLight.position.set(0, Chunk.SEA_LEVEL, 0);
    this.hemisphereLight.castShadow = false;
    this.scene.add(this.hemisphereLight);

    this.ambientLight = new THREE.AmbientLight(new THREE.Color(0xd0e7fe), 0.5);
    ...
    this.moonBoundLight = new THREE.SpotLight(new THREE.Color(0xc5dadd), 0.2, 0, Math.PI / 2, 1.0);
    this.moonBoundLight.castShadow = false;
    this.moonBoundLight.target = target;
    this.scene.add(this.moonBoundLight);

    this.sunBoundLight = new THREE.SpotLight(new THREE.Color(0xe86b4a), 0.8, 0, Math.PI / 2, 1.0);
    ...
    (<THREE.Mesh>this.sun.children[0]).material = new THREE.MeshLambertMaterial({ color: new THREE.Color(0xffec83), emissive: new THREE.Color(0x505050), emissiveIntensity: 1.0, reflectivity: 0.5 });
    ...
    (<THREE.Mesh>this.moon.children[0]).material = new THREE.MeshLambertMaterial({ color: new THREE.Color(0x83d8ff), emissive: new THREE.Color(0x505050), emissiveIntensity: 1.0, reflectivity: 0.5 });
    ...
    if (configSvc.debug) {
      ...
      this.lightHelper = new THREE.ArrowHelper(dirHelper, this.sunlight.position.clone(), this.sunRadius, new THREE.Color(0xff0000), 10000);
  ...
  private initSunlight() {
    const d = 500000;
    this.sunlight = new THREE.DirectionalLight(new THREE.Color(0xffffff), 0.275);
  ...
  private initMoonlight() {
    // const d = 500000;
    this.moonlight = new THREE.DirectionalLight(new THREE.Color(0x5fc2eb), 0.125);
  ...
  private updateSun(delta: number) {
    if (this.sunRevolutionTime !== 0) {
      this.sunAngle += THREE.MathUtils.degToRad(360 / this.sunRevolutionTime) * delta;
    }
  ...
  private updateLights() {
    ...
    const k: number = Math.floor(THREE.MathUtils.radToDeg(this.sunAngle % MathUtils.TWO_PI) * 10) / 10;
    ...
    this.hemisphereLight.color = this.computeSkyColor(k, phaseInfo); // was skyColor property that no longer seems supported?
  ...
  private getPhaseInfo(rad: number): IPhaseInfo | null {
    const angle = THREE.MathUtils.radToDeg(rad % MathUtils.TWO_PI);
  ...
  private watchStartTime() {
    multiplayerSvc.time$.subscribe(startTime => {
      ...
      const diffAngle = THREE.MathUtils.degToRad(360 / this.sunRevolutionTime) * diff;
  ...
  */
  ...
  getSunAngle(): number {
    return THREE.MathUtils.radToDeg(Math.atan2(this.sun.position.y, this.sun.position.x));
  }
