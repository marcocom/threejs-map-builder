...
class Terrain {
  ...
  preload() {
    ...
    // borders generation
    const bt1 = this.getBorderMesh(1, Terrain.NCOLS, (row, col) => col * Chunk.CELL_SIZE_X, () => 0);
  ...
  */
  ...
  private getBottomMesh(): THREE.Mesh {
    ...
    geometry.vertices.push(new THREE.Vector3(0, -Terrain.SIZE_Y / divider, 0));
    geometry.vertices.push(new THREE.Vector3(Terrain.SIZE_X, -Terrain.SIZE_Y / divider, 0));
    geometry.vertices.push(new THREE.Vector3(Terrain.SIZE_X, -Terrain.SIZE_Y / divider, Terrain.SIZE_Z));
    geometry.vertices.push(new THREE.Vector3(0, -Terrain.SIZE_Y / divider, Terrain.SIZE_Z));

    const f1 = new THREE.Face3(0, 1, 2);
    const f2 = new THREE.Face3(2, 0, 3);
    f1.color = this.generator.getSubBiome(-Terrain.SIZE_Y / divider / Chunk.MAX_TERRAIN_HEIGHT, 0).color;
    f2.color = this.generator.getSubBiome(-Terrain.SIZE_Y / divider / Chunk.MAX_TERRAIN_HEIGHT, 0).color;
    geometry.faces.push(f1);
    geometry.faces.push(f2);
