...
class Terrain {
  ...
  preload() {
    ...
    if (World.GENERATE_WATER && biome.hasWater()) {
      (<THREE.BufferGeometry>this.water.geometry).mergeMesh(bw1);
      (<THREE.BufferGeometry>this.water.geometry).mergeMesh(bw2);
      (<THREE.BufferGeometry>this.water.geometry).mergeMesh(bw3);
      (<THREE.BufferGeometry>this.water.geometry).mergeMesh(bw4);
      ...
      (<THREE.ShaderMaterial>this.water.material).uniforms.water_distortion.value =
        configSvc.config.ENABLE_WATER_EFFECTS && !window.isFreezed && biome.getWaterDistortion();
  ...
  update(frustum: THREE.Frustum, position: THREE.Vector3, delta: number) {
    ...
    if (!window.isFreezed && biome.hasWater()) {
      // update water distorsion effect
      (<THREE.ShaderMaterial>this.water.material).uniforms.time.value += 1 * delta;
  ...
  */
  ...
  getWaterBorderMesh(
    ...
    for (let col = 0; col < nbVerticesZ; col++) {
      for (let row = 0; row < nbVerticesY; row++) {
        ...
        geometry.setAttribute('position', new THREE.BufferAttribute(float, 3));
  ...
  private getBottomMesh(): THREE.Mesh {
    const geometry = new THREE.BufferGeometry();
    const vertices = [];
    vertices.push(new THREE.Vector3(0, -Terrain.SIZE_Y / 2, 0));
    vertices.push(new THREE.Vector3(Terrain.SIZE_X, -Terrain.SIZE_Y / 2, 0));
    vertices.push(new THREE.Vector3(Terrain.SIZE_X, -Terrain.SIZE_Y / 2, Terrain.SIZE_Z));
    vertices.push(new THREE.Vector3(0, -Terrain.SIZE_Y / 2, Terrain.SIZE_Z));
    const float = new Float32Array(vertices);
    geometry.setAttribute('position', new THREE.BufferAttribute(float, 3));
  ...
  private initMeshes() {
    ...
    this.terrain = new THREE.Mesh(new THREE.BufferGeometry(), TERRAIN_MATERIAL);
    this.terrain.frustumCulled = true;
    this.terrain.castShadow = true;
    this.terrain.receiveShadow = true;
    this.layers.add(this.terrain);

    this.terrainSide = new THREE.Mesh(new THREE.BufferGeometry(), TERRAIN_SIDE_MATERIAL);
    ...
    // water
    this.water = new THREE.Mesh(new THREE.BufferGeometry(), WATER_MATERIAL);
  ...
  static createRegionBoundingBoxHelper(bbox: THREE.Box3 = null): THREE.Box3Helper {
    return new THREE.Box3Helper(bbox ? bbox : Terrain.createRegionBoundingBox(), new THREE.Color(0xff0000));
  }
  ...
  static createRegionWaterBoundingBoxHelper(bbox: THREE.Box3 = null): THREE.Box3Helper {
    return new THREE.Box3Helper(bbox ? bbox : Terrain.createRegionWaterBoundingBox(), new THREE.Color(0x0000ff));
  }
}
