...
class Chunk {
  ...
  private readonly bbox: THREE.Box3;
  private readonly bboxHelper: THREE.Box3Helper;
  ...
  saveObject(obj: THREE.Object3D) {
    console.log(`Chunk.saveObject() Object3D:`, obj);
    const saveObj:IPick = Chunk.convertObjectToPick(obj);
    console.log(`Chunk.saveObject() IPick:`, saveObj);
    this.savePick(saveObj);
    console.log(`Chunk.saveObject() >>  this.objects:[IPick]`, this.objects);
  }
  ...
  canPlaceObject(object: THREE.Object3D): boolean {
    ...
    for (const item of this.objects.children) {
      const bbox2 = new THREE.Box3().setFromObject(item);
  ...
  */
  ...
  static createBoundingBoxHelper(bbox: THREE.Box3): THREE.Box3Helper {
    return new THREE.Box3Helper(bbox, new THREE.Color(0xffff00));
  }
  ...
  static createBoundingBoxHelperFromCoords(row: number, col: number): THREE.Box3Helper {
    return new THREE.Box3Helper(Chunk.createBoundingBox(row, col), new THREE.Color(0xffff00));
  }
