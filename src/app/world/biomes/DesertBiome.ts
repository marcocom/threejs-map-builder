...
class DesertBiome extends Biome {
  ...
  private pyramid: THREE.Object3D;
  ...
  private initSpecialObject() {
    ...
    this.pyramid = this.terrain.placeSpecialObject({
      stackReference: 'pyramid',
      float: false,
      underwater: ISpecialObjectCanPlaceIn.LAND
    }, centerX - sizeX / 2 + 30000, centerZ - sizeZ / 2 - 50000, sizeX, sizeZ);
  }
  ...
  computeElevationAt(x: number, z: number): number {
    const nx = (x - Terrain.SIZE_X / 2) / (1024 * 192);
    const nz = (z - Terrain.SIZE_Z / 2) / (1024 * 256);

    let e = 0.225 * this.generator.noise2(1.5 * nx, 1.5 * nz);
    e += 0.2 * this.generator.noise3(1.5 * nx, 1.5 * nz);
    e += 0.075 * this.generator.ridgeNoise(1 * nx, 1 * nz);
    e += 0.00985 * this.generator.ridgeNoise(1 * nx, 1 * nz);
    e += 0.008 * this.generator.noise2(2 * nx, 2 * nz);
    e += 0.075 * this.generator.noise(2 * nx, 2 * nz);
  ...
  computeMoistureAt(x: number, z: number): number {
    ...
    return Math.max(value - 0.25, 0);
  }

  getParametersAt(e: number, m: number): IBiome {
    if (e < Chunk.SEA_ELEVATION + 0.03) {
      return SUB_BIOMES.OASIS_WATER;
    }

    if (e > Chunk.SEA_ELEVATION + 0.06) {
