...
class Home extends React.PureComponent<IHomeProps, IHomeState> {
  ...
  constructor(props) {
    ...
    if (photo) {
      ...
      if (storage.quality === GraphicsQuality.PHOTO)
        storage.quality = GraphicsQuality.HIGH;
    }
    ...
    coreSvc.init().then(
      () => {
        this.setState({ ready: true },
          () => {
            if (this.state.autostart) this.goToLoading();
          });
      },
      (e) => console.log('home.componentDidMount unfulfilled error:', e));
  }
  ...
  getSubmitHTML(): JSX.Element {
    ...
    return coreSvcIsInitialized
      ? (
        <Button form='gameSetup' type='submit' className='btn--orange-secondary btn--expand-mobile btn--big btn--rounded' disabled={busy}>
