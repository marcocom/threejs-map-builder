...
class TrophiesTab extends React.Component<ITrophiesTabProps, ITrophiesTabState> {
  ...
  async componentDidMount() {
    ...
    await this.trophySubscription.unsubscribe();
    this.sortBy();
  }
    ...
    return (
      <div className='tab trophies-tab'>
        <header className='tab__header mb-2'>
          <Row suffix='-48'>
            <Col className='flexcol--12-t mb-2 mb-0-t'><H3 className='color-theme'>{translationSvc.translate('UI.trophies-tab.title')}</H3></Col>
            <Col className='flexcol--12-t'>{this.renderSelect()}</Col>
          </Row>
        </header>
        <div className='tab__content'>
          ...
          <footer className='tab__footer progression-reset'>
            <Row>
              ...
              <Col className='flexcol--24 flex justify-content--begin'>
                <Button className='btn--red btn--expand-mobile btn--rounded' onClick={this.resetProgression}>{translationSvc.translate('UI.trophies-tab.reset_btn')}</Button>
              </Col>
            </Row>
          </footer>
        </div>
