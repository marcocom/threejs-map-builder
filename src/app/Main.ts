...
class Main {
  ...
  constructor() {
    ...
    this.camera = new THREE.PerspectiveCamera(40, aspect, near, far); //this moves camera start distance after world is created. 1000 is far 1 is close. 
  }
    ...
    if (PointerLock.available) {
      ...
      document.body.addEventListener('keydown', e => {
        if (this.world.isInitialized()) {
          this.world.handleKeyboard(e.key, this.controls.enabled);
        }
      });
